...
static int ring3_event_held; /* valid only for RING3_EVENT_CLICK -- see syscall.h */
 ...
 * `BACKBUFFER_ADDR` (0x200000) and clear of `PROGRAM_LOAD_ADDR` (now
 * 0x00340000 -- `BACKBUFFER_ADDR` and `PROGRAM_LOAD_ADDR` used to
 * coincidentally share the same address, a real collision fixed by
 * relocating `PROGRAM_LOAD_ADDR`; see its own comment above), and does
 * not need to be inside the one 4MB page directory entry
 * `paging_set_user()` ever makes user-accessible, since only ring-0
 * code (this file, syscall_gfx.c) ever touches it directly. */
 ...
 * programs/hello/hello.ld and programs/paint/paint.ld (and any future
 * program's own linker script) link against.
 *
 * Was 0x00200000 -- the same address as kernel/gfx/graphics.c's own
 * BACKBUFFER_ADDR -- until Task 5 of docs/superpowers/sdd/2026-08-29-
 * standalone-paint's verification actually caught the collision this
 * caused: a loaded program's entire memory image (its .bss included)
 * was physically aliased on top of the live video backbuffer
 * (0x200000-0x32C000 at this codebase's fixed 640x480), so every
 * gfx_fill_rect()/gfx_put_pixel() syscall any window issued -- the
 * kernel's own desktop redraws included, since ring3_wait_event()
 * re-drives kmain_frame() on every SYS_WAIT_EVENT call -- silently
 * scribbled over the program's own static state. Moved to 0x00340000,
...
static void kmain_frame(void) {
        ...
        while (mouse_poll_packet(&dx, &dy, &buttons)) {
...

            {
                 ...
                 *
                 * Level-triggered on left_held, not click_edge -- fires
                 * every frame the button is down over the window, not
                 * just the initial press, so a program doing continuous
                 * work while the button stays down (PAINT dragging to
                 * paint a stroke of cells, not just one per click) sees
                 * a fresh event each frame. held distinguishes the
                 * initial press (0, same frame click_edge would have
                 * fired) from every later frame of the same hold (1),
                 * for programs that need press-only semantics for some
                 * hit regions (a button) and continuous semantics for
                 * others (a canvas) -- see struct sys_wait_event_args's
                 * own doc-comment in syscall.h. */
                if (ring3_is_topmost && left_held &&
                    ...
                    ring3_event_x = cx;
                    ring3_event_y = cy;
                    ring3_event_held = prev_left_held;
                }
 ...
 * docs/superpowers/specs/2026-08-29-ring3-window-events-design.md. */
void ring3_wait_event(int *type, int *x, int *y, char *key, int *held) {
...
void kmain(void) {
     ...
     * (see kmain()'s start-menu click dispatch), not auto-run at boot. */
    {
        static const unsigned char paint_bin[] = {
            
